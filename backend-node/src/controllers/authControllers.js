...
export const socialAuthSuccess = (req, res) => {
  ...
  req.session.save((err) => {
    if (err)
      return res.redirect(`${process.env.FRONTEND_URL}/src/views/auth/login.html?error=session_error`);

    supabase.from('users').update({ otp_verified: true }).eq('id', req.user.id).then(() => {});
