/**
 * controllers/assignmentControllers.js
 */
...
export async function createAssignment(req, res) {
  try {
    ...
    const result = await query(
      `INSERT INTO assignments (tl_id, clan_id, scope, title, module_id, content_type, storage_path, repo_url, file_name, deadline)
    ...
    if (scope === 'all') {
      ...
      coderResult = await query(`SELECT id FROM users WHERE role = 'coder' AND clan = $1 AND is_active = true`, [clan]);
    }

    for (const coder of coderResult.rows) {
      await notifyUser(coder.id, `Nueva actividad: ${title}`, `Tu TL ${tlName} publicó una nueva actividad.`, 'assignment', assignmentId);
    }
    await notifyUser(tl.id, `Actividad Publicada`, `La actividad "${title}" ha sido asignada.`, 'assignment', assignmentId);
...
export async function listAssignmentsTL(req, res) {
  try {
    const tl = req.user;
    const result = await query(
      `SELECT a.*, m.name AS module_name FROM assignments a
       LEFT JOIN modules m ON a.module_id = m.id
       WHERE a.tl_id = $1 AND a.is_active = true ORDER BY a.created_at DESC`,
      [tl.id]
    );
...
export async function deleteAssignment(req, res) {
  try {
    ...
    if (!result.rows.length) return res.status(404).json({ error: 'Actividad no encontrada o sin permisos.' });
...
export async function getAssignmentDownload(req, res) {
  try {
    ...
    const clan = coderResult.rows[0]?.clan;
    const result = await query(
      `SELECT * FROM assignments WHERE id = $1 AND is_active = true AND (scope = 'all' OR (scope = 'clan' AND clan_id = $2))`,
      [id, clan]
    );
    if (!result.rows.length) return res.status(404).json({ error: 'No encontrado.' });
    const assignment = result.rows[0];
    if (!assignment.storage_path) return res.status(400).json({ error: 'Esta actividad no tiene PDF.' });
    const { data, error } = await supabase.storage.from(BUCKET).createSignedUrl(assignment.storage_path, 3600);
